...
local function __Class(TypeName, SuperType)
    ...
    if SuperType then
        ClassType.super = setmetatable({}, 
        {
            __index = function (t, k)
                local Func = __ClassTypeList[SuperType][k]
    ...
    if SuperType then
        ...
        {
            __index = function (t, k)
                local Ret = __ClassTypeList[SuperType][k]
                Vtbl[k] = Ret
                return Ret
            end
        })
...
-- Set into global.
if (not IsDeclared("Class")) or (not Class) then
    Declare("Class", __Class)
end
